...
export async function crimsonRite({speaker, actor, token, character, item, args, scope, workflow}) {
    ...
    if (!selection) return;
    let riteMenu = [];
    if (targetActor.items.getName('Rito Carmesim: Rito das Chamas')) riteMenu.push(['Rito das Chamas', 'fire']);
    if (targetActor.items.getName('Rito Carmesim: Rito do Congelamento')) riteMenu.push(['Rito do Congelamento', 'cold']);
    if (targetActor.items.getName('Rito Carmesim: Rito da Tempestade')) riteMenu.push(['Rito da Tempestade', 'lightning']);
    if (targetActor.items.getName('Rito Carmesim: Rito dos Mortos')) riteMenu.push(['Rito dos Mortos', 'necrotic']);
    if (targetActor.items.getName('Rito Carmesim: Rito do Oráculo')) riteMenu.push(['Rito do Oráculo', 'psychic']);
    if (targetActor.items.getName('Rito Carmesim: Rito do Rugido')) riteMenu.push(['Rito do Rugido', 'thunder']);
    if (targetActor.items.getName('Rito do Amanhacer')) riteMenu.push(['Rito do Amanhacer', 'radiant']);
    let damageType;
